...
The histogram above is about the tourist % net food consumption. Note that there is just one country (**Albania**) with the strange value.

````{r, echo=FALSE}
hist(100*consumpCal$percentNet, breaks = 100, xlab = "Tourist % Netconsumption", main="")
````

Let's check the number of tourists flow from Albania in 2010, 2011 and 2012.
This plot show the tourist flow growth percentage from 5 countries in 3 years. So,
we can see that is no make sense the values for Albania.
...
p <- ggplot(data=top5TouristFlow, aes(x=as.factor(Country), y=percentIncreasing, 
  ...
  theme(legend.position="none") +  ylab('growth %') + xlab('country') + 
  #theme_fao(45) +
  annotate("text", x = 1, y = 20000, label = "46,000%") +
