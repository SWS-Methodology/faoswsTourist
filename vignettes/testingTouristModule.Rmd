...
coords <- lapply(slot(fao_world, "polygons"), function(x){
    lapply(slot(x, "Polygons"), function(y) slot(y, "coords"))
})
countryName <- fao_world$ADM0_NAME
coords <- lapply(coords, function(x){
    out = data.frame(do.call("rbind", x))
    colnames(out) = c("longitude", "latitude")
    out
})
coords <- lapply(1:length(coords), function(i){
    coords[[i]]$country = as.character(countryName[i])
    coords[[i]]
})
coords <- do.call("rbind", coords)
ggplot(coords, aes(x = longitude, y = latitude, group = country)) +
    geom_line()
```