...
consumpCal.1 <- consumpCal[is.na(totalCalNetCountry) != T & is.na(totalCalInCountry) != T & is.na(totalCalOutCountry) != T & 
             totalCalInCountry != 0 & totalCalOutCountry != 0]
...
consumpCal.2 <- consumpCal[is.na(totalCalOutCountry) == T | is.na(totalCalInCountry) == T | totalCalInCountry == 0]
...
There are information for 70 countries (37.4%) and the others countries are missing data.
...
````{r include=FALSE, cache=FALSE}
consumpCal <- consumpCal[order(-consumpCal$pop)]
````

Country   |Population (million) | % Net Tourist Consumption
----------- | -----|--------
 United States of America                 |         317.5| -0.03%
 Brazil       |         198.8|0.04%
 Pakistan     |         179.1|-0.001%
 Nigeria             |         168.8|-0.02% 
 Russian Federation(1992-)                  |         143.1|-0.14%

## Tourist consumption for the 5 smallest countries by population

According to a table below, the 5 smallest countries has positive net tourist consumption.


Country   |Population (million) | % Net Tourist Consumption
----------- | -----|--------
 Saint Kitts and Nevis                 |         54| 3.38%
 Bermuda       |         65|4.33%
 Seychelles     |         92|5.04%
 Grenada             |         105|2.02% 
 Saint Vincent and the Grenadines                  |         109|1.31%
...
The table above show the number of countries in each dataset.

DataSet     | Numbe of Countries
--------------------- | -------------
 Tourist Flow     |         144
 Food Data |         103 
 Tourist Flow & Food Data   |         70


````{r include=FALSE, cache=FALSE}
options(warn=-1)
## Number of countries with  tourist flow data : #144
#length(unique(tourismDays.6[year == 2012 & is.na(daysNet) != T & daysIn != 0 & daysOut != 0 &  
 #                             is.na(daysIn) != T & is.na(daysOut) != T])$country)

## Number of countries with food data? # 103
#calorieConsumptionData.7.1[, list(length(unique(country))),
 #                          by = list(year)]

## Number of countries with 2 information

#consumpCal[is.na(totalCalNetCountry) != T & is.na(totalCalInCountry) != T & is.na(totalCalOutCountry) != T & 
 #            totalCalInCountry != 0 & totalCalOutCountry != 0]


grid.newpage()
draw.pairwise.venn(144, 103, 70, category = c("Tourist Flow Data", "Food Data"), lty = rep("blank", 
                                                                                   2), fill = c("skyblue", "orange"), alpha = rep(0.5, 2),cat.pos = c(0,0), cat.dist = rep(0.025, 2))
